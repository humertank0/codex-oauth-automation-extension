...
- 如果当前邮箱来源可自动获取，整套流程更接近全自动
- 当 `Mail = Burner Mailbox` 时，Auto 会优先尝试直接生成 Burner 邮箱；如遇人机验证，会暂停等待你处理后继续
- 当 `Mail != Burner Mailbox` 时，Auto 会先重试 Duck 自动获取 5 次；仍失败时，才会在邮箱阶段暂停
...
- 如果侧边栏邮箱为空：
  - 当 `Mail = Burner Mailbox` 时，会先尝试自动生成 Burner 邮箱
  - 其他模式下，会先尝试自动获取 DuckDuckGo 邮箱
  - 如果自动获取失败，再提示手动粘贴
- 自动填写邮箱
...
Burner 通过 `content/burner-mail.js`：

- 打开 `https://burnermailbox.com/mailbox`
- 点击 `New`
- 生成新的随机邮箱地址
- 轮询 Burner 收件箱中的验证码邮件

这个功能会被：

- 侧边栏 `Email` 旁边的 `获取` 按钮使用
- `Auto Run` 流程优先尝试使用
...
如果 Duck / Burner 页面打不开、未登录、被验证拦住或按钮变化：

- 直接在 `Email` 输入框中粘贴邮箱
- 手动点 `Step 3` 时，如果邮箱为空，脚本会先自动尝试获取当前 provider 对应邮箱；失败后再改为手填
...
- Burner Mailbox 可能偶尔触发人机验证，需要手动过一下再继续
